...
当前仓库描述的是 1.5.0 代码状态。模型是否可用、接口是否收费、是否需要 VPN，始终以用户配置的真实 API 和网络环境为准；README 不把目录中的模型名、价格或免费额度当成服务承诺。
...
1.5.0 在保留完整工作区 Agent 能力的同时，集中完成了以下升级：

- **更快的输入与流式链路。** 根据模型和实测吞吐组织请求，批量传递高频事件，复用可并发的 OpenCode 内核运行时，减少长上下文、快速输出和多任务并行时的界面阻塞。
- **更稳的任务恢复。** 断流支持最多 5 次重连；未执行工具的瞬时请求失败可以有界重试；停滞看门狗、内核退出检测、运行恢复和资源清理避免任务无期限转圈或拖垮桌面进程。
- **连接式 API 配置。** 用户可以建立任意数量的连接，保存名称、Base URL、API Key、兼容预设和媒体端点；测试连接后直接选择返回模型，也支持手填私有模型 ID。
- **可选子代理协作。** Explore、Review、Research、Test 与 Build 角色可按需启用，主任务仍持有最终交付权，权限、工作区和并发槽位均受 Yan Kernel 约束。
- **完整桌面个性化。** 主界面和设置支持中英文切换、壁纸市场与自定义壁纸、用户名称与口吻、模型选择器、引导文档和更清晰的任务状态。
...
- **直接使用模型最终回复。** OpenCode 自身回合完成后，内核直接采用模型原生的最终用户回复，不再额外发起一次无工具总结请求；内部工作过程会自动折叠，但仍可随时展开核对。
...
- 最多进行 6 轮目标验收。达到稳定通过、用户要求结束，或达到上限后，当前验收回合直接给出最终回复并说明证据和未解决项，不再追加总结回合。
...
1.5.0 继续提供 **Yan Computer Use** 测试能力。Yan 使用轻量适配层连接官方 Nuphus MCP，只向内核暴露原生 `desktop_*` 工具，隐藏 Nuphus 自带的网页工具以避免与 Yan 内置浏览器冲突。配套 Skill 约束 Agent 先激活目标窗口，再进行视觉理解与精确坐标感知，执行操作后按界面变化复查，最后完成针对性视觉验收。

真实桌面工具开始执行时，屏幕边缘会显示独立蓝色动态光效，顶部显示“Yan Agent正在操控你的电脑，按Esc退出”；按 Esc 可中止当前电脑操控。通用视觉中继仍然保留。该能力仍处于测试阶段，桌面软件的可访问性、界面结构和 Nuphus 感知质量会影响结果，不能保证自动化所有应用。
...
- 用户可以保存任意数量的独立 API 连接。官方、第三方中转和私有部署分别保存名称、Base URL、API Key、兼容预设和模型目录，主文本、图片、视频角色也分别保留自己的连接身份；切换文本连接不会清空另一连接下已经选择的图片或视频模型。
- 每个连接可以手填模型 ID，用于目录接口不可用或私有部署端点的场景。动态发现或手工填写只表示目录已配置，不保证远端端点、请求格式、权限和额度可用。
...
GLM 与 SenseNova 的网络提示按当前应用配置为国内直连，Agnes 通常需要 VPN；API 仍受用户网络、服务状态、额度和限流影响。视觉中继只报告它实际从图片看到的事实，不替代主模型执行任务，也不把图片中的文字当成工具指令。

### 9. 输出、总结与审阅

- 工作阶段的正文、思考摘要、工具调用和工具结果都来自 OpenCode 事件，不再依赖旧的占位流程文本。
- DSML 协议片段会在输出层过滤；最终用户看见的是正常正文和工具状态，而不是 `read`、`write` 等协议标记。
- 工作完成后隐藏中间工作内容，只显示模型原生最终回复；输出底部的“查看工作过程”药丸按钮会展开同一套正文 UI，不创建另一套独立输出面板。
- 代码块提供图标化的全部复制按钮。
...
- 设置页包括 API 配置、模型库存、视觉中继、权限、口吻、快速启动和关于。
- API 配置是一张用户自建连接列表：每条连接填写名称、Base URL、API Key 和可选的生图/编辑图/生视频 POST 地址，一键测试返回模型列表即可保存。兼容预设（DeepSeek DSML、Anthropic、GLM、通义等）默认按名称和 URL 自动识别，也可手动指定。旧版按厂商分组的配置会在首次启动时自动导入为连接。
...
- 内置终端使用真实 PTY；当前界面标签为 PowerShell。PowerShell 7 专属运行时不属于 1.5.0 的承诺范围。
- 右侧面板提供辅助对话、内置浏览器、文件与 Git 工作区视图；底部用户、当前主题、宠物和设置入口采用独立的纵向控制。

#### 二代 Yan Agent Pet

1.5.0 的桌面宠物是第二代 Yan Agent Pet，不再是与内核脱节的静态装饰：
...
- 视觉中继、图片生成和视频生成的结果会进入工作结果与最终回复，成功生成图片不会自动再调用 `read_image` 消耗一次读图请求。
...
以下是 1.5.0 代码内置或由 Yan 管理的主要 MCP。实际暴露给某次运行的工具集合还会受任务类型、工作区、权限、Skill 和 API 配置影响。
...
1.5.0 的安装版和便携版应以项目的 GitHub Releases 页面为准；在正式发布前不要使用 README 中旧版本的下载地址：
...
- Agent 的最终回复只报告已观察到的工具结果和工作区事实；无法验证的内容会标为未验证或失败。

## 已知限制与后续方向

1.5.0 仍有明确边界：

- **Yan Computer Use 仍是测试能力，不是全应用自动化承诺。** 它依赖 Nuphus MCP、目标软件界面和视觉感知质量；复杂 Canvas、自绘界面、权限窗口或频繁变化的桌面仍可能失败。
- **Yan Work GUI 仍在开发。** 当前入口属于开发预览，不建议把它当成可交付的生产界面。
- 1.5.0 已提供按角色启用的任务内子代理，但常驻子 Agent、无限动态并发、托管平台 PR/Issue 全流程和内置 Git 凭据管理尚未纳入稳定承诺；本地 Git 工作区操作已经集成。
