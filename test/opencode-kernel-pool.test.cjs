...
test('pool cancellation reaches runs waiting for kernel acquisition', async t => {
  const kernels = [];
  ...
  const pool = new OpenCodeSidecar({
    ...
    kernelFactory: () => {
      const kernel = new FakeKernel(`kernel-${kernels.length + 1}`);
      if (!kernels.length) kernel.startGate = startGate;
      kernels.push(kernel);
      return kernel;
    }
  });
  ...
  const first = pool.run({ runId: 'run-first', openCodeConfig: { model: 'vendor/first' } });
  await waitFor(() => kernels.length === 1);
  const second = pool.run({ runId: 'run-second', openCodeConfig: { model: 'vendor/second' } });
  await waitFor(() => pool.pendingPooledRuns?.has('run-second'));
  const secondOutcome = second.then(() => null, error => error);

  assert.deepEqual(await pool.cancel('run-second'), {
    ok: true,
    requested: true,
    pending: true,
    settled: false
  ...
  startGate.resolve();
  await waitFor(() => pool.hasRun('run-first'));
  kernels[0].finish('run-first');
  await first;
