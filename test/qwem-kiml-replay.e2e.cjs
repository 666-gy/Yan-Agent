...
const { OpenCodeSidecar, buildOpenCodeConfig, stageQwemProviderModule, stageKimlProviderModule } = require('../lib/opencode-sidecar');
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    try {
      ...
      assert.equal(body.reasoningEffortAdjusted, undefined, 'internal adjustment metadata must not reach API');
      if (body.model === 'qwen3.8-max') assert.equal(body.reasoning_effort, 'medium');
      if (body.model === 'kimi-k3') assert.equal(body.reasoning_effort, 'high');
      ...
      const childThought = '  Child reasoning\n精确回传。\n';
...
(async () => {
  ...
  try {
    ...
    for (const modelId of ['qwen3.8-max', 'kimi-k3']) {
      const providerId = 'replay-fixture';
      const config = buildOpenCodeConfig({ providerId, modelId,
        qwemProviderModule: stageQwemProviderModule({ appRoot, dataDir: path.join(root, 'data') }),
        kimlProviderModule: stageKimlProviderModule({ appRoot, dataDir: path.join(root, 'data') }),
    ...
    assert.ok(counts.child >= 8 && counts.replay > 0, JSON.stringify(counts));
    console.log(JSON.stringify({ ok: true, counts, exactParentReasoning: true, exactChildReasoning: true, followup: true, models: ['qwen3.8-max', 'kimi-k3'] }));
  } finally {
