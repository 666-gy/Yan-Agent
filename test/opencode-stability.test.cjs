...
test('classifies gateway request-size failures as recoverable context overflow', () => {
  const overflow = [
    'Request Entity Too Large',
    'HTTP 413: payload too large',
    'content too large for upstream gateway'
  ];
  for (const message of overflow) {
    assert.equal(isContextOverflowError({ message }), true, message);
  }
  assert.equal(isContextOverflowError({ message: 'Invalid API key provided' }), false);
});
