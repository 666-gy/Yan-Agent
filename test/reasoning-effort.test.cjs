...
(async () => {
  ...
  // 必须显式映射到最近支持档（平局向上）并标记 adjusted。
  ...
  assert.equal(resolveReasoningForModel('glm-5.3[1m]', 'medium').effort, 'high');
  assert.equal(resolveReasoningForModel('vendor/glm-5.3[1m]', 'medium').effort, 'high');
  assert.equal(resolveReasoningForModel('glm-5.30[1m]', 'medium').adjusted, false);
  assert.equal(resolveReasoningForModel('glm-5.4[1m]', 'medium').adjusted, false);
