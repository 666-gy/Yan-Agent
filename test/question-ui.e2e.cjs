...
(async () => {
  ...
  try {
    ...
    assert.equal(optionViewport.rows.slice(0, 3).every(row => row.bottom <= optionViewport.viewport.bottom + 1), true, JSON.stringify(optionViewport));
    ...
    assert.ok(geometry.panelHeight <= 212, JSON.stringify(geometry));
    ...
    assert.equal(geometry.toggleHeight, 26, JSON.stringify(geometry));
    assert.equal(geometry.customHeight, 26, JSON.stringify(geometry));
    assert.equal(geometry.skipHeight, 26, JSON.stringify(geometry));
    ...
    for (const viewport of [
      ...
      const responsive = await page.locator('#agentPermissionPanel').evaluate((panel, expectedViewport) => {
        ...
        return {
          ...
          firstThreeOptionsVisible: optionRows.length === 3 && optionRows.every(rect => (
      ...
      assert.equal(responsive.firstThreeOptionsVisible, true, JSON.stringify(responsive));
