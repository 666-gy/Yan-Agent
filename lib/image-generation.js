...
function resolveImageEndpoint({ baseUrl, providerId, strategy, model, isEdit = false }) {
  const root = String(baseUrl || '').replace(/\/$/, '');
  if (strategy === 'responses') return `${root}/responses`;
  const usesAi8Images = (providerId === 'grok' && isGrokImageGenerationModel(model))
...
async function generateImage({
  ...
  try {
    ...
    const endpoint = resolveImageEndpoint({ baseUrl, providerId, strategy, model, isEdit });
