'use strict';

const { decorateModels, resolveModelCapabilities } = require('./model-capabilities');
const {
  GLM_VISION_RELAY_MODELS,
  SENSENOVA_OFFICIAL_SUPPLEMENTAL_MODELS
} = require('./provider-model-catalog');

const AGNES_FALLBACK_MODELS = Object.freeze([
  { id: 'agnes-2.0-flash', name: 'Agnes 2.0 Flash' },
  { id: 'agnes-2.5-flash', name: 'Agnes 2.5 Flash' },
  { id: 'agnes-2.5-pro', name: 'Agnes 2.5 Pro' },
  { id: 'agnes-2.5-pro-alpha', name: 'Agnes 2.5 Pro Alpha' },
  { id: 'agnes-image-2.0-flash', name: 'Agnes Image 2.0 Flash' },
  { id: 'agnes-image-2.1-flash', name: 'Agnes Image 2.1 Flash' },
  { id: 'agnes-video-v2.0', name: 'Agnes Video V2.0' }
]);

const GLM_FREE_VISION_RELAY_MODELS = GLM_VISION_RELAY_MODELS;
const SENSENOVA_VISION_RELAY_MODELS = Object.freeze(SENSENOVA_OFFICIAL_SUPPLEMENTAL_MODELS
  .filter(model => model.modelType === 'text' && model.capabilities?.vision === true));
const SILICONFLOW_VISION_RELAY_MODELS = Object.freeze([
  { id: 'Qwen/Qwen3.5-4B', name: 'Qwen/Qwen3.5-4B', modelType: 'text', capabilities: { vision: true } },
  { id: 'deepseek-ai/DeepSeek-OCR', name: 'deepseek-ai/DeepSeek-OCR', modelType: 'text', capabilities: { vision: true } },
  { id: 'PaddlePaddle/PaddleOCR-VL-1.5', name: 'PaddlePaddle/PaddleOCR-VL-1.5', modelType: 'text', capabilities: { vision: true } }
].map(model => Object.freeze(model)));
const VISION_RELAY_PRESET_ORDER = Object.freeze(['glm', 'sensenova', 'agnes', 'siliconflow']);
const VISION_RELAY_MODELS_BY_PRESET = Object.freeze({
  glm: GLM_FREE_VISION_RELAY_MODELS,
  sensenova: SENSENOVA_VISION_RELAY_MODELS,
  agnes: Object.freeze([
    AGNES_FALLBACK_MODELS.find(model => model.id === 'agnes-2.5-flash'),
    AGNES_FALLBACK_MODELS.find(model => model.id === 'agnes-2.0-flash')
  ].filter(Boolean)),
  siliconflow: SILICONFLOW_VISION_RELAY_MODELS
});

const DEFAULT_MODEL_ROLES = Object.freeze({
  text: { providerId: 'agnes', model: 'agnes-2.0-flash' },
  image: { providerId: 'agnes', model: 'agnes-image-2.1-flash' },
  video: { providerId: 'agnes', model: 'agnes-video-v2.0' }
});

const AGNES_RECOMMENDED_MODEL_IDS = Object.freeze([
  DEFAULT_MODEL_ROLES.text.model,
  DEFAULT_MODEL_ROLES.image.model,
  DEFAULT_MODEL_ROLES.video.model
]);

function getModelType(providerId, model = {}) {
  const explicit = String(model?.capabilities?.modelType || model?.modelType || '').toLowerCase();
  if (['text', 'image', 'video'].includes(explicit)) return explicit;
  return resolveModelCapabilities(providerId, model).modelType;
}

function isAgentTextModel(providerId, model = {}) {
  if (getModelType(providerId, model) !== 'text') return false;
  const outputs = model.outputModalities || model.output_modalities || model.capabilities?.output_modalities || model.capabilities?.output;
  if (Array.isArray(outputs) && outputs.length && !outputs.some(value => String(value).toLowerCase() === 'text')) return false;
  const id = String(model.id || '').toLowerCase();
  return !/(?:embedding|rerank|moderation|transcri|whisper|speech|tts|realtime|dall-e|image-generation|video-generation|stable-diffusion|sdxl|kolors|flux|cogvideo|hunyuanvideo|(?:^|[\/_-])sora(?:[\/_-]|$)|(?:^|[\/_-])wan\d)/i.test(id);
}

function configuredSupplierModels(models, configured) {
  return configured ? (Array.isArray(models) ? models : []) : [];
}

function buildQuickModelEntries(provider = {}, selected = {}, supplier = null, selectedSupplier = true) {
  const providerId = String(provider?.providerId || '').trim();
  if (!providerId || !provider?.configured) return [];
  const supplierId = String(supplier?.supplierId || supplier?.id || '').trim();
  const supplierName = String(supplier?.supplierName || supplier?.name || supplierId).trim();
  const catalog = decorateModels(providerId, provider.models || []);
  return catalog.filter(model => {
    const modelType = getModelType(providerId, model);
    return modelType === 'text' ? isAgentTextModel(providerId, model) : ['image', 'video'].includes(modelType);
  }).map(model => ({
    providerId,
    providerName: provider.providerName || providerId,
    ...(supplierId ? { supplierId, supplierName: supplierName || supplierId } : {}),
    id: model.id,
    name: model.name || model.id,
    modelType: getModelType(providerId, model),
    source: model.source || 'api',
    selected: selectedSupplier
      && selected.providerId === providerId
      && String(selected.modelId || selected.model || '') === model.id
      && (selected.modelType || 'text') === getModelType(providerId, model),
    requiresApiKey: true,
    capabilities: model.capabilities || {},
    configured: true
  }));
}

function buildQuickModelList({ providers, activeSelection, activeProviderId, activeTextModel } = {}) {
  const selected = activeSelection && typeof activeSelection === 'object'
    ? activeSelection
    : {
        providerId: activeProviderId,
        modelId: activeTextModel,
        modelType: 'text'
      };
  return (Array.isArray(providers) ? providers : [])
    .flatMap(provider => buildQuickModelEntries(provider, selected));
}

function buildQuickSupplierGroups({
  providers,
  activeSelection,
  activeProviderId,
  activeSupplierId
} = {}) {
  const selected = activeSelection && typeof activeSelection === 'object'
    ? activeSelection
    : {
        providerId: activeProviderId,
        modelId: '',
        modelType: 'text'
      };
  const selectedProviderId = String(selected.providerId || activeProviderId || '').trim();
  const selectedSupplierId = String(activeSupplierId || '').trim();

  return (Array.isArray(providers) ? providers : []).flatMap(provider => {
    const providerId = String(provider?.providerId || '').trim();
    if (!providerId) return [];
    const suppliers = (Array.isArray(provider.suppliers) ? provider.suppliers : []).flatMap(supplier => {
      const supplierId = String(supplier?.supplierId || supplier?.id || '').trim();
      if (!supplierId || !supplier?.configured) return [];
      const isSelected = providerId === selectedProviderId
        && supplierId === (selectedSupplierId || (supplier.active ? supplierId : ''));
      const publicSupplier = {
        supplierId,
        supplierName: String(supplier.supplierName || supplier.name || supplierId).trim() || supplierId,
        active: !!supplier.active,
        selected: isSelected
      };
      const models = buildQuickModelEntries({
        providerId,
        providerName: provider.providerName || providerId,
        configured: true,
        models: supplier.models || []
      }, selected, publicSupplier, isSelected);
      return [{ ...publicSupplier, models }];
    });
    if (!suppliers.length) return [];
    return [{
      providerId,
      providerName: provider.providerName || providerId,
      selected: suppliers.some(supplier => supplier.selected),
      suppliers
    }];
  });
}

function buildMediaModelList({
  providers,
  currentProviderId,
  currentProviderName,
  currentProviderModels,
  currentProviderApiKey,
  agnesModels,
  agnesApiKey,
  media
} = {}) {
  const candidates = Array.isArray(providers)
    ? providers.flatMap(provider => decorateModels(provider.providerId, provider.models || []).map(model => ({
        providerId: provider.providerId,
        providerName: provider.providerName || provider.providerId,
        supplierId: provider.supplierId || '',
        supplierName: provider.supplierName || '',
        model,
        configured: !!provider.configured
      })))
    : decorateModels('agnes', agnesModels?.length ? agnesModels : AGNES_FALLBACK_MODELS)
        .map(model => ({ providerId: 'agnes', providerName: 'Agnes', model, configured: !!String(agnesApiKey || '').trim() }));
  if (!Array.isArray(providers) && currentProviderId !== 'agnes' && String(currentProviderApiKey || '').trim()) {
    candidates.push(...decorateModels(currentProviderId, currentProviderModels || []).map(model => ({
      providerId: currentProviderId,
      providerName: currentProviderName || currentProviderId,
      model,
      configured: true
    })));
  }
  const seen = new Set();
  return candidates.flatMap(entry => {
    // A media catalog is only user-visible when the supplier still has a
    // real credential. This final guard protects callers that pass a cached
    // or otherwise stale catalog entry.
    if (!entry.configured) return [];
    const modelType = getModelType(entry.providerId, entry.model);
    const key = `${entry.providerId}:${entry.supplierId || ''}:${entry.model.id}`;
    if (!['image', 'video'].includes(modelType) || seen.has(key)) return [];
    seen.add(key);
    return [{
      providerId: entry.providerId,
      providerName: entry.providerName,
      supplierId: entry.supplierId || '',
      supplierName: entry.supplierName || '',
      id: entry.model.id,
      name: entry.model.name || entry.model.id,
      modelType,
      selected: media?.[`${modelType}Provider`] === entry.providerId
        && media?.[`${modelType}Model`] === entry.model.id
        && (!media?.[`${modelType}SupplierId`] || media?.[`${modelType}SupplierId`] === entry.supplierId),
      configured: entry.configured,
      source: entry.model.source || 'api',
      capabilities: entry.model.capabilities || {}
    }];
  });
}

module.exports = {
  AGNES_FALLBACK_MODELS,
  AGNES_RECOMMENDED_MODEL_IDS,
  DEFAULT_MODEL_ROLES,
  GLM_FREE_VISION_RELAY_MODELS,
  GLM_VISION_RELAY_MODELS,
  SENSENOVA_VISION_RELAY_MODELS,
  SILICONFLOW_VISION_RELAY_MODELS,
  VISION_RELAY_MODELS_BY_PRESET,
  VISION_RELAY_PRESET_ORDER,
  buildMediaModelList,
  buildQuickModelList,
  buildQuickSupplierGroups,
  configuredSupplierModels,
  getModelType,
  isAgentTextModel
};
