  ...
  const provider = modelID ? {
    [providerID]: {
      ...
      npm: usesDeepSeekDsml
        ...
        : (apiFormat === 'anthropic' ? '@ai-sdk/anthropic' : '@ai-sdk/openai-compatible'),
      options: {
        ...(apiKey ? { apiKey } : {}),
...
function settledAssistantSince(messages, submittedAt) {
...
function completedAssistantSince(messages, submittedAt) {
...
function successfulAssistantSince(messages, submittedAt) {
  return (Array.isArray(messages) ? messages : [])
    .filter(message => (
      message?.info?.role === 'assistant'
      && !message?.info?.error
      && Number(message.info.time?.created) >= Number(submittedAt || 0) - 1_000
      && Number(message.info.time?.completed) >= Number(submittedAt || 0)
    ))
    .sort((left, right) => messageCompletedAt(left) - messageCompletedAt(right))
    .at(-1) || null;
}
...
function skillStorageSystem(request = {}) {
  ...
  return [
    ...
    'To invoke an installed Skill, call the Yan Skills read_skill tool and apply the returned exact instructions immediately.',
...
function availableSkillsSystem(request = {}) {
  ...
  const skipped = (Array.isArray(request.skippedSkills) ? request.skippedSkills : []).filter(skill => skill?.id);
  ...
  return [
    ...
    `For every read_skill call in this run, pass task_id=${String(request.runId || 'unknown-run')} exactly so Yan can enforce the per-task retry threshold.`,
...
function finalSummaryPrompt(originalPrompt, skippedSkills = []) {
  const skipped = (Array.isArray(skippedSkills) ? skippedSkills : []).filter(skill => skill?.id);
  return [
    ...
    skipped.length
      ? `The following Skills failed to load and were skipped. Briefly disclose each name, retry count, and reason in the final answer; do not claim they were used:\n${skipped.map(skill => `- ${skill.name || skill.id}: ${Number(skill.attempts) || 1} attempt(s); ${skill.error || 'unknown load error'}`).join('\n')}`
      : '',
...
function collectRunResult(messages, baselineIDs, diffs, todos, request, openCodeSessionID, summaryState = false) {
  ...
  const assistantText = lastTextParts
...
class OpenCodeSidecar {
  ...
  async run(request = {}, onEvent = () => {}) {
    ...
    try {
      ...
      const sendPrompt = async (prompt, system, options = {}) => {
        ...
        const settled = await this.#waitForIdle(session.id, directory, submittedAt, abortController.signal, run);
        ...
        return { submittedAt, messages: promptMessages, settledAssistantID: settled?.assistantID || '' };
      };
      ...
      const lastWorkAssistant = (run.loopGuardAssistantID
        ? freshAssistants.find(message => message?.info?.id === run.loopGuardAssistantID)
        : null) || freshAssistants.at(-1);
      const incompleteGoalTodos = request.workMode === 'goal'
        && !run.finishRequested
      ...
      const runSummaryRound = async () => {
        ...
        do {
          ...
          const summaryPrompt = await sendPrompt(
            finalSummaryPrompt(request.prompt, request.skippedSkills),
  ...
  async analyzeInterjection(payload = {}, onEvent = () => {}) {
    ...
    try {
      ...
      onEvent({ type: 'error', data: { message: error?.message || String(error) } });
      throw error;
    } finally {
  ...
  async cancelInterjection(runId, requestId = '') {
    ...
    if (this.client && request.sessionID) {
      try {
        ...
        return { ok: false, cancelled: false, error: error?.message || String(error) };
      }
    }
  ...
  async #waitForIdle(sessionID, directory, submittedAt, signal, run) {
    ...
    while (true) {
      ...
      if (status && status.type !== 'idle') {
        sawBusy = true;
        const messages = unwrap(await this.client.session.messages({ sessionID, directory, limit: 24 }), 'OpenCode loop guard check');
        const settled = settledAssistantSince(messages, submittedAt);
        if (settled) {
          ...
          if (
            ...
            try {
              ...
              throw new Error(`OpenCode stop-loop guard failed: ${error?.message || String(error)}`);
            }
          }
        } else {
          settledAssistantID = '';
      ...
      if (!status || status.type === 'idle') {
        ...
        const completed = completedAssistantSince(messages, submittedAt);
        if (completed) {
          const assistantID = run?.loopGuardAssistantID || '';
          run.loopGuarded = false;
          return { assistantID };
        }
  ...
  async #waitForFinalText(sessionID, directory, submittedAt, signal) {
    ...
    while (Date.now() <= deadline) {
      ...
      assistant = successfulAssistantSince(messages, submittedAt) || latestAssistantSince(messages, submittedAt);
