  ...
  return new Promise((resolve, reject) => {
    ...
    socket.on('data', chunk => {
      ...
      try {
        ...
        if (!response?.ok) finish(new Error(response?.error || 'Yan built-in browser bridge failed.'));
...
function toolDefinitions() {
  const tools = [
    ...
    },
    {
      ...
      description: 'Inspect the current Yan browser page and return fresh element refs, roles, labels, states, and visible text. Call this before click or type, and call it again after navigation or major page changes.',
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        properties: {
          ref: { type: 'string', description: 'Fresh element ref such as e3.' },
