...
const { buildPatchRows } = require('./run-change-summary');
...
async function repositoryStatus(workspace) {
  ...
  const [porcelain, currentBranchResult, headResult, upstreamResult, branches, remotes, userName, userEmail, operation, numstatResult] = await Promise.all([
    runGit([
  ...
  let diffStats = parseNumstat(numstatResult.ok ? numstatResult.stdout : '');
  if (!numstatResult.ok && !head) {
    const [stagedStats, unstagedStats] = await Promise.all([
      tryGit(['-C', root, 'diff', '--cached', '--numstat', '--no-renames', '--', '.', ...INTERNAL_PATHSPECS], { readOnly: true }),
      tryGit(['-C', root, 'diff', '--numstat', '--no-renames', '--', '.', ...INTERNAL_PATHSPECS], { readOnly: true })
    ]);
    const staged = parseNumstat(stagedStats.ok ? stagedStats.stdout : '');
    const unstaged = parseNumstat(unstagedStats.ok ? unstagedStats.stdout : '');
    diffStats = {
      added: staged.added + unstaged.added,
      deleted: staged.deleted + unstaged.deleted,
      binaryFiles: staged.binaryFiles + unstaged.binaryFiles
    };
  }
  let ahead = 0;
  let behind = 0;
...
function addedFileRows(content) {
  return String(content || '').replace(/\r\n?/g, '\n').split('\n').filter((_, index, lines) => (
    index < lines.length - 1 || lines[index] !== ''
  )).map((text, index) => ({ type: 'add', oldLine: null, newLine: index + 1, text }));
}

async function review(workspace) {
  const root = await resolveRepository(workspace);
  const status = await repositoryStatus(root);
  const headResult = await tryGit(['-C', root, 'rev-parse', '--verify', 'HEAD'], { readOnly: true });
  const files = await Promise.all(status.changes.map(async change => {
    let rows = [];
    let additions = 0;
    let deletions = 0;
    let binary = false;

    if (change.status === 'untracked' || !headResult.ok) {
      if (change.status !== 'deleted') {
        ...
        if (!binary) {
          rows = addedFileRows(content.toString('utf8'));
      ...
      const [patchResult, numstatResult] = await Promise.all([
        tryGit([
          '-C', root, 'diff', 'HEAD', '--no-ext-diff', '--no-color', '--unified=3', '--', change.path
        ], { readOnly: true, maxBuffer: MAX_OUTPUT_BYTES }),
        tryGit([
          '-C', root, 'diff', 'HEAD', '--numstat', '--no-renames', '--', change.path
        ], { readOnly: true })
      ]);
      const stats = parseNumstat(numstatResult.ok ? numstatResult.stdout : '');
      additions = stats.added;
      deletions = stats.deleted;
      binary = stats.binaryFiles > 0;
      if (!binary && patchResult.ok) rows = buildPatchRows(patchResult.stdout).rows;
    }
