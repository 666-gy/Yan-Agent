...
async function readDirectSkills(root) {
  ...
  for (const entry of entries) {
    ...
    try {
      const parsed = skillRegistry.parseSkillDocument(await fsp.readFile(skillFile, 'utf8'), {
...
async function installOneCliSkill(candidate, source, replaceExisting) {
  ...
  try {
    ...
    const parsed = skillRegistry.parseSkillDocument(await fsp.readFile(path.join(prepared, 'SKILL.md'), 'utf8'), {
      requireFrontmatter: true,
      requireName: true,
      requireDescription: true,
      requirePrompt: true
...
async function readInstalledSkill(input) {
  ...
  const loadPromise = skillRegistry.readSkillWithRetry(
...
function toolDefinitions() {
  return [
    ...
    {
      name: 'read_skill',
      description: 'Load the complete instructions for any available Yan Agent Skill, including bundled and user-installed Skills. Use it before specialized work when the Skill catalog matches the task and immediately after installing a Skill.',
      inputSchema: {
        type: 'object',
        properties: {
