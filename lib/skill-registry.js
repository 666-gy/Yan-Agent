...
function parseSkillDocument(text) {
  ...
  if (lines[0]?.trim() !== '---') return { metadata: {}, prompt: normalized.trim() };
  let closing = -1;
  ...
  if (closing < 0) return { metadata: {}, prompt: normalized.trim() };
  ...
  return {
    metadata,
    prompt: lines.slice(closing + 1).join('\n').trim()
  };
}
...
function readSkill(skillId, taskContext, cfg, appRoot, dataDir, saveConfig, options = {}) {
  syncSkillStore(cfg, appRoot, dataDir);
  const stored = skillStore.resolveSkillStore(dataDir, skillId);
  let skill = stored?.skill || null;
  let fuzzy = !!stored && !stored.exact;
  if (!skill) {
