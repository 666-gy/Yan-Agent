...
function buildScreenshotRelayInput({ basePrompt = '', question = '', compare = false, previous = null, current = null } = {}) {
  ...
  const currentFrame = current?.data
    ? { name: 'yan-browser-screenshot-current.png', mimeType: String(current.mimeType || ''), data: String(current.data) }
    : null;
  ...
  const attachments = withPrevious
    ? [{
        name: 'yan-browser-screenshot-previous.png',
        mimeType: String(previous.mimeType || ''),
        data: String(previous.data)
      }, currentFrame]
    : (currentFrame ? [{ name: 'yan-browser-screenshot.png', mimeType: currentFrame.mimeType, data: currentFrame.data }] : []);
