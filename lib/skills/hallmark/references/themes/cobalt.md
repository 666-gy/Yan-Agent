...
Loaded eagerly by SKILL.md Step 3 whenever the catalog pick is `cobalt`. The OKLCH palette + font stack live in [`theme-tokens.css`](../theme-tokens.css) under `[data-theme="cobalt"]`. Canonical build: [upstream cobalt-01](https://github.com/Nutlope/hallmark/tree/main/site/examples/cobalt-01) (an API product landing with a request/response hero).
...
Plus the Space Grotesk + Inter + JetBrains Mono link and the small reveal/type-in script. The canonical build to mirror is [upstream cobalt-01](https://github.com/Nutlope/hallmark/tree/main/site/examples/cobalt-01).
