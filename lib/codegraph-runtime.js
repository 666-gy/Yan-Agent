...
function packagedRuntimeRoot(appRoot) {
  ...
  if (path.basename(root).toLowerCase() !== 'app.asar') return '';
  return path.join(path.dirname(root), 'codegraph-runtime');
}

function runtimeFromRoot(candidate) {
  const command = path.join(candidate, 'node.exe');
...
function resolveRuntime(appRoot) {
  ...
  try {
    ...
    return { ok: false, error: `未找到 CodeGraph Windows 运行时：${error.message}` };
  }
}
