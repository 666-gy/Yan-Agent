...
async function resolveLocalImagePath(value, requestedWorkspace = '') {
  ...
  if (requested && registryEntries.length && !registryEntries.some(entry => (
    ...
    throw new Error('指定工作区不是当前 Yan 任务的工作区');
  }
    ...
    tools.push({
      ...
      inputSchema: {
        ...
        properties: {
          ...
          workspace: { type: 'string', description: 'Optional absolute workspace path. Required when multiple Yan tasks run concurrently.' },
