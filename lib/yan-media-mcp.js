...
const { detectImageType, generateImage } = require('./image-generation');
const { generateVideo } = require('./video-generation');
const { describeImages, isRecoverableVisionRelayError } = require('./vision-relay');
...
async function resolveLocalImagePath(value) {
  ...
  const workspace = String(access.workspace || '').trim();
  const candidate = path.resolve(path.isAbsolute(raw) ? raw : path.join(workspace || process.cwd(), raw));
  ...
  if (String(access.accessMode || '') !== 'full') {
    if (!workspace) throw new Error('当前未选择工作区；请传入 Yan 已生成图片的 generated_image_id，或先选择工作区');
    const resolvedWorkspace = await fsp.realpath(workspace).catch(() => path.resolve(workspace));
    if (!isInsideDirectory(resolvedWorkspace, resolved)) throw new Error('图片不在当前工作区内，无法读取');
  }
  return resolved;
}
...
function toolDefinitions() {
  ...
  if (runtime.image) {
    tools.push({
      name: 'generate_image',
      description: 'Generate an image with the configured secondary image model. A successful result completes generation and must be summarized directly without a routine read_image call. For revisions of an image generated earlier in this Yan conversation, pass its generatedImageId as source_asset_id without reading it first. Never ask the user to download and re-upload a Yan-generated image.',
      inputSchema: {
        type: 'object',
        properties: {
