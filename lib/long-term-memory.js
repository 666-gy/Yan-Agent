...
function upsertIntoStore(store, record) {
  ...
  if (duplicate) {
    if (!sameSourceRun(duplicate.source, record.source)) {
      duplicate.occurrences = Math.max(1, Number(duplicate.occurrences) || 1) + 1;
    }
...
class LongTermMemoryStore {
  ...
  query({ query = '', workspace = '', maxChars = DEFAULT_MAX_CONTEXT_CHARS, limit = 12 } = {}) {
    ...
    const scored = candidates
      ...
      .map(memory => ({ memory, score: scoreMemory(memory, queryTokens, normalizedQuery, workspace) }))
